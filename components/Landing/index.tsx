...
const slides: Array<Slide> = [
  {
    ...
    title: "Listen Here",
    content: "Find your inner passion for music through rhythm, melody, and harmony"
  },
  {
    color: "bg-dark-pink",
...
export default function Landing() {
  const {next, prev, getSlide, getPrev, getNext, slideIndex} = useSliderControl(slides);
  // grid-template-rows-[1fr_min-content_max-content]
  // grid-rows-1 
  return (
      ...
      <div className="flex flex-row justify-start items-start gap-5 h-fit w-full col-start-1 order-2 lg:order-none lg:col-span-1 lg:w-fit lg:justify-center lg:items-center lg:h-full">
        <div className="w-full flex flex-col justify-start items-start gap-12 pr-0 max-w-[86vw] lg:pr-8 lg:max-w-xl">
          ...
          <div className="w-full flex flex-row justify-start">
            <button className="px-6 py-3 font-circular text-base text-white uppercase bg-black rounded-full w-2/3 font-semibold tracking-wide hover:bg-[#1d1d1d]">
              Login
            </button>
          </div>
        </div>
